...
5. To sync vocals to character lip movement (music-video shots only): chop the chosen track into ~12s slices (Joey's sweet spot per the K-Pop breakdown; 15s is the ceiling). Upload each slice to the Seedance host's audio reference — Higgsfield calls it the "elements list," ArtCraft calls it "Audio Ref" (3 slots × 15s total). The per-shot lyric is written into the Seedance prompt body by `cinema-worldbuilder` as *"lips visibly mouthing the words '[exact lyric]' with exaggerated clarity"* — see `ai-film-director` Step 5.0.5 (Phase 5) for the orchestration.
