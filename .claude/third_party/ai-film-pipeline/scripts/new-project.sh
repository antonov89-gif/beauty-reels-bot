...
# --- NOTE: story-bible.md is intentionally NOT stubbed here. ---
# video-qa and the ai-film-director orchestrator use the existence of
# projects/<name>/story-bible.md as the "a story bible was built" signal.
# A placeholder stub would falsely trip that check. The file only gets
# created when the producer actually runs the story-bible-builder skill
# (say "build a story bible").
...
2. **Open this folder** in Claude Code, or open claude.ai with the five skills installed.
...
| Path | Purpose |
|---|---|
| \`story-bible.md\` | Optional world + character canon (premise, timeline, factions, locations, per-character voice/movement/stillness locks) — created by the story-bible-builder skill when you run the interview — say "build a story bible" |
...
echo "  ├── story-bible.md                 ← not created — build via story-bible-builder skill"
