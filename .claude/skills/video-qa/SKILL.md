...
This skill is **drift screening, not authoritative drift detection.** An LLM comparing a compressed frame still to a locked character sheet (3-panel default, or the legacy 6-panel) is unreliable on subtle identity drift:
...
- **Pro QC (opt-in)** — three checks only: drift screening + continuity audit + prompt rule scan (plus an optional fourth, voice/dialogue-register drift, when the project has a story bible and the clip has dialogue). Pause with evidence when something fires, fix iteratively, move on. No score. No verdict. No regression mode. No WTF-likelihood. For producers who know what they're looking for and want a lean audit.
...
> - **Pro** → three checks only (drift screening, continuity audit, prompt rule scan), plus an optional fourth (voice/dialogue-register drift) when the project has a story bible and dialogue's in scope. I pause when something fires, you fix iteratively. No score, no report. Recommended for in-flight checks during a generation session.
>
...
## PRO MODE — THREE CHECKS, NO SCORE (PLUS AN OPTIONAL VOICE-DRIFT CHECK)

In pro mode, the skill runs three focused checks, plus a fourth that only switches on when it applies. No tier picker, no severity weighting, no health score, no verdict. The producer gets surfaced findings with evidence and decides what to do.
...
3. **Prompt rule scan.** Read `generation-log.md` and scan every Seedance prompt for: music/lyric language, real brand names, proper character names, age descriptors ("young," "teen," etc.). Pure text scan. Highly reliable because it's not visual judgment. Flag every hit; the producer decides.
   ...
   **What's changed with cinema-worldbuilder 3.0 — don't flag these as violations:**
   - **Element tags** (`@driver_ref`, `@venue_plate`, and other user-named `@tag` references) are correct prompt grammar, not a naming leak. Never flag an `@tag` the way a proper name would be flagged.
   - **FOV written in degrees with mm in parentheses** (e.g. `47° (50mm)`) is the expected camera-language format. A prompt using mm-only with no degree anchor is the thing to flag (as a Cinema mode coherence finding, not a prompt-rule violation) — not the reverse.
   - **Character reference plates on a flat, shadowless 18% gray (or white, if explicitly requested) seamless** are correct per `banana-pro-director`'s locked default. Don't flag the flat, evenly-lit register on a *reference plate* as a photoreal-stack problem — only flag flatness that leaks into a *scene* clip that should have real lighting.
   - **The prompt's code block should be English-only.** Flag any non-English content inside it as a rule violation.
   - **A Seedance prompt should carry the ten labeled blocks** (Scene & Mood → Frame Map → Subject Lock(s) → Cross-Frame Rules → Movement → Last Frame → World Plate → Sound Bed → Capture Realism → Camera Capture). A prompt missing one of these structurally is worth flagging — it's a format regression, not a style choice.
   - Real brand names, proper character names, and age descriptors are still forbidden — unchanged.

4. **Voice / dialogue-register drift (optional — fires only when both conditions hold: a `story-bible.md` exists for this project, and the clip(s) in scope carry dialogue or lip-sync).** Compare the delivered voice/dialogue register — cadence, phrasing, register, the felt texture of how the line lands — against that character's locked **Speech** line in the story bible. This is a lighter-touch check than the visual ones: it's reading a description against a rendered performance, not pixel comparison, so treat findings as candidates for the producer's ear, not a definitive call. Evidence-based pause like the other checks — quote the locked Speech line + describe what the clip actually delivers. Fix path: regenerate with the Speech lock explicitly restated in `cinema-worldbuilder`'s **Sound Bed** block — in-frame spoken dialogue is diegetic audio, and Sound Bed's own rule permits dialogue physically spoken in frame — with the character's Movement/Stillness locks carried in the Subject Lock / Movement blocks. Skip this check entirely — don't mention it — when no story bible exists for the project or nothing in scope has dialogue.

### Pro mode behavior

- **No tiers.** Pro mode runs the checks every time (three, plus the fourth when it applies). No Quick/Standard/Exhaustive selector.
...
> Pro QC — checks coming: drift screening (per character/prop), continuity audit (from your shot list), prompt rule scan (music/brand/name/age leaks, plus format conformance, from your gen log)[, and a voice-drift check against your story bible, since this project has one and there's dialogue in scope — only asked when both are true]. Upload whatever's in scope and I'll surface findings as I go.

Then run the checks in whatever order makes sense for what's been uploaded.
...
**(A) The full project workspace** — `shot-list.md`, `generation-log.md`, `story-bible.md` (if the project has one), every `character-bible-*.md`, `environment-bible-*.md`, `prop-bible.md`, all `references/` images, all `clips/` videos. The skill walks the project end-to-end.
...
- **Story bible (if present):** note which characters have a locked Speech / Movement / Stillness entry in `story-bible.md` — this is what the voice-drift check (below) and the character-consistency Movement comparison read against.
...
- **Clips:** for each, note shot number, character(s) + look(s) in frame, environment, props, assigned cinema mode, runtime, and whether the shot carries dialogue or lip-sync.
...
- [ ] Compare side-by-side with the character's locked sheet. **The chest-up face-lock panel is the primary identity anchor** — it's the panel banana-pro-director's 3-panel default (Mode 2A) tunes specifically for face fidelity, so lead the comparison there before checking the full-body front/rear panels. If the character sheet is the legacy 6-panel, use its dedicated face panel the same way.
...
- [ ] **If a story bible exists and this shot has dialogue or lip-sync:** does the delivered voice/dialogue register match the character's locked Speech line (register, cadence, phrasing)? Fold this in as part of the character-consistency read, not a separate pass — a character whose face is locked but whose voice drifts is still drifting. See the voice-drift check under PRO MODE for the evidence-and-fix-path pattern; apply the same pattern here.

Common severities:
- **Critical:** different bone structure / different person reads
- **High:** hair color shifted, eye color shifted, wrong look entirely, voice/dialogue register unrecognizable against the locked Speech line
- **Medium:** subtle hair texture shift, one wardrobe element missing/wrong, voice register noticeably off but still readable as the same character
...
- [ ] Does the lens character match? (Anamorphic flares + oval bokeh for M1/M3/M4; clean spherical with natural round bokeh and even sharpness for M2.)
- [ ] Is the FOV written in degrees with mm in parentheses (e.g. `47° (50mm)`), picked from cinema-worldbuilder's discrete anchor-step ladder (180° / 107° / 84° / 63° / 47° / 29° / 18° / 12° / 8°) rather than an off-ladder value or mm-only? Off-ladder or mm-only lens callouts are a prompt-conformance finding here, not a visual one.
- [ ] Does the grade match? (Teal-amber M1; saturated editorial, warm-retained blacks M2; color-negative film with heavier low-light grain M3; desaturated cool with warm bloom M4; palette-driven M5.)
- [ ] Atmospheric detail match? (Haze in M3/M4; clean in M2; weathered material in M5.)
- [ ] Does the prompt behind this clip carry all ten of cinema-worldbuilder's labeled blocks (Scene & Mood → Frame Map → Subject Lock(s) → Cross-Frame Rules → Movement → Last Frame → World Plate → Sound Bed → Capture Realism → Camera Capture)? A missing block is a format regression worth flagging even if the clip itself looks fine.

Common severities:
- **High:** wrong mode entirely (an M1 narrative grade applied to what should be M3 action — register collapses)
- **Medium:** right mode, wrong FOV (M1 at `18° (100mm)` when the shot calls for `84° (24mm)` establishing); missing a required prompt block
- **Low:** subtle grade drift inside the right mode; off-ladder FOV value close to the intended anchor
...
- [ ] Grain: visible color-negative film grain (fine 35mm-style, per mode) — or scrubbed clean?
...
- [ ] Any proper names in the prompt body? **Forbidden.** Even if the user uses them in chat, the prompt text must describe by visual marker. **Exception:** `@tag`-style element tags (`@driver_ref`, `@venue_plate`, etc.) are legitimate cinema-worldbuilder 3.0 reference grammar, not proper names — never flag a `@tag` here.
...
- [ ] Is the prompt's code block English-only? Non-English content inside it is a rule violation.
...
- `--prompts-only` — scan `generation-log.md` prompts without needing the clips (catches brand/name/music issues and cinema-worldbuilder 3.0 format conformance — missing blocks, off-ladder FOV, non-English content — purely from text; `@tag` element references are not flagged)
- `--voice-only` — voice/dialogue-register drift only, scoped to dialogue-bearing clips against `story-bible.md`'s locked Speech lines (only meaningful when the project has a story bible)
...
3. **Prompt rule scan on the prompt actually used** for this generation — music language? real brand names? proper character names? age descriptors? format conformance (ten blocks present, FOV on-ladder, English-only)? Text check, fast and reliable. `@tag` element references are not violations — see PRO MODE check 3 for the full rule list.
4. **Voice/dialogue-register check (only if this shot has dialogue or lip-sync and the project has a `story-bible.md`)** — compare against the character's locked Speech line. Skip silently otherwise.
...
Before the mode question, confirm the specialist skills are installed — the **fix paths** depend on `banana-pro-director`, `cinema-worldbuilder`, and (when the project has one) `story-bible-builder` activating in this conversation. If a fix needs a regenerated prompt and the specialist isn't installed, the orchestrator falls back to generic Claude output and the fix degrades quietly.

Open with:

> Quick install check first — when I flag a drift, the fix path involves handing back to `banana-pro-director` (image prompts) or `cinema-worldbuilder` (Seedance prompts) to rewrite the prompt that produced the slip; a voice/dialogue-register finding hands back to `story-bible-builder` (or the character's locked bible entry) to restate the Speech lock. These need to be installed in claude.ai (Settings → Capabilities → Skills) alongside this one. Without them, the fix prescriptions degrade to generic Claude prompt-writing — which loses the locked photoreal stack, cinema-mode grammar, and voice locks. Zara from the source production team: *"Use the skill, not the web version. That's the whole difference."*
>
> **Quick check — are `banana-pro-director` and `cinema-worldbuilder` installed? (`story-bible-builder` too, if this project has a story bible.)**
>
> - **Yes, installed** → continuing to the mode question.
> - **No / not sure** → install what's missing from claude.ai Settings → Capabilities → Skills before we go further. Better to install now than ship a "fixed" clip that wasn't actually fixed.
...
> - **Pro** → three checks only (drift screening, continuity audit, prompt rule scan), plus an optional fourth (voice/dialogue-register drift) when the project has a story bible and dialogue's in scope. No score, no report. Use for in-flight checks while you're still generating.
>
> If unsure, **guided** is the safe default.
...
> 1. `shot-list.md`
> 2. `generation-log.md`
> 3. `story-bible.md`, if this project has one
> 4. Character bibles + locked character sheets (3-panel default, or legacy 6-panel) for every recurring character
> 5. Environment bibles + plates for every recurring location
> 6. Prop bible + sheets
> 7. Generated clips (or frame stills extracted from them)
...
> 4. `story-bible.md`, if this project has one and any of the new work carries dialogue

**For specific shots:**

> Running guided QC at [tier] on Shot(s) [#]. Pause-with-evidence on findings, no full report.
>
> Inventory check — drop in: the clip(s) for the listed shots, the prompts used (from `generation-log.md`), the bibles for any characters / environments / props in those shots, and `story-bible.md` if those shots carry dialogue and the project has one.

Then run the eight-category sweep per the workflow, scoped to what was uploaded.

**If pro:** open the check ask:

> Pro QC — checks coming: drift screening, continuity audit, prompt rule scan[, and a voice-drift check against your story bible, if this project has one and dialogue's in scope]. No score, no report, just findings as I go.
>
> Upload whatever's in scope right now — clips you just generated, locked references, `generation-log.md` excerpts, and `story-bible.md` if relevant. I'll surface what fires.

Then run the pro checks against what's been uploaded. Pause-and-fix the moment something Critical or High fires. Don't write a report.
