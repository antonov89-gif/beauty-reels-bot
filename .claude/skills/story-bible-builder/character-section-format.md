...
1. **The four prompt-ready descriptors (Speech, Movement, Stillness, Suno) must be in quotes.** They're engineered to drop verbatim into future prompts — either as standalone prompt inputs, or as feeder blocks for a video prompt director skill (like `cinema-worldbuilder`). Specifically:
