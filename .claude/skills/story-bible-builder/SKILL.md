...
Many users pair the bible with a video prompt director skill (in this pipeline, `cinema-worldbuilder`). In this pairing, the director skill handles the cinematography grammar, mode selection, frame composition, and prompt syntax. The bible provides the identity, voice, movement, aesthetic era locks, and canon that the director skill can't get from a reference image alone.
...
Reference `character-section-format.md` for how each character section is structured. Reference `example-bible-excerpts.md` when the user needs to see what a good section looks like.
...
Assistant behavior: never invent. Mark `[TBD]` if the user doesn't know. Push on aesthetic locks — this is where most bibles are weakest. When a user says "dark and moody," ask "what colors specifically?" Reference the demo excerpts in `example-bible-excerpts.md` if the user needs to see the level of specificity.
...
For each character, run the character interview in `character-interview.md`. It covers:
...
1. If a video prompt director skill is also active in the session (in this pipeline, `cinema-worldbuilder`), the director skill handles cinematography, mode selection, frame composition, and prompt syntax
2. The bible feeds the director skill: character voice → Sound Bed; movement/stillness → Subject Lock; aesthetic era locks → World Plate / grade; ensemble dynamics → Cross-Frame Rules; production rules → cross-frame rules and locked traits
...
Ask the user what production companion skills they use (if any) and name them explicitly in this closing section so future Claude knows the paired workflow. In this pipeline, the usual companions are `cinema-worldbuilder` (video prompt director — reads the bible's voice/movement/aesthetic/ensemble locks per the mapping above) and `banana-pro-director` (visual identity and character/environment plates), with `ai-film-director` orchestrating the overall session across skills.

**Where to save the file:**
- **In Claude Code (this repo):** save one bible per project to `projects/<project-name>/story-bible.md`, alongside that project's `character-bible-*.md`, `environment-bible-*.md`, `shot-list.md`, etc. Present the path once saved.
- **On claude.ai:** save to `/mnt/user-data/outputs/[working-title-slug].md` and present it. Offer to zip it as an installable `.skill` file if the user wants that packaging.
...
3. **Push on the vague.** "Dark and moody" isn't an aesthetic lock. "Cool voice" isn't a voice profile. "Powerful" isn't a description. When the user gives a vague answer, ask the follow-up that makes it prompt-actionable — color specifics, register, movement quality. Reference `example-bible-excerpts.md` if the user needs to see the level of specificity.
...
- `character-section-format.md` — Structure for each character section in the final output
- `character-interview.md` — Questions to walk each character through
- `example-bible-excerpts.md` — Sample sections at the right density level to reference or show the user
