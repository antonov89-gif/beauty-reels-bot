...
The prompts this skill produces are the same regardless of where you run them. Before defaulting to Higgsfield's Banana Pro / Soul Cinema (which costs per-credit and can run up fast on face locks and character sheets that need 10–20 rerolls), **strongly consider running image generation in ChatGPT first**.
...
- **Identity-preserving edits.** ChatGPT's current image model is exceptionally good at *only changing what the user asks to change*. Drop in an existing character image and say *"make a 3-panel character sheet of this person"* — the character's face, build, and styling stay intact across all three panels. This is the killer feature for the character-sheet step, where Banana Pro often drifts the face from panel to panel and forces multiple rerolls.
...
The prompt grammar this skill produces (locked identity paragraph, cinema stack, no-naming rule, brand-neutral language, 3-panel default sheet structure, scene plate composition) **transfers directly** to ChatGPT. The user pastes the same prompt into a ChatGPT conversation. The cinema stack still does its job. The identity lock still holds. The no-naming rule still applies. Nothing changes about what this skill writes.

What changes for the user is the *workflow*:

- **Mode 0 (face lock):** This is now the first ChatGPT-run step for any new character. Paste the banana-pro-director face-lock prompt into ChatGPT. Optionally upload an inspiration photo first. Iterate via natural-language tweaks until the face lands. Save the locked image to `references/characters/<working-name>/base.png`.
- **Mode 1 (base outfit):** Upload the locked face reference from Mode 0 and describe the outfit. Paste the Mode 1 prompt. Iterate conversationally until the fit reads right. Save the result as the base outfit reference.
- **Mode 2 (character sheet):** This is where ChatGPT's identity-preservation shines. Upload the locked base image and either (a) paste the full 3-panel prompt or (b) just say *"make a 3-panel character sheet of this person — headless front, full rear, tight chest-up face lock"* (the locked 3-panel default; only ask for a 6-panel if that legacy format is specifically wanted). The character stays intact across panels.
- **Mode 3 (scene plates / props):** Upload locked character + environment inspiration. Paste the scene-plate prompt. Edit conversationally to refine composition.
...
> *"Heads up — running this in ChatGPT (recommended for cost) or Higgsfield?"*

Match the pause-and-save grammar to their answer. *"Run this in ChatGPT now. Upload the reference image, paste the prompt, iterate until the face lands. Save the locked image to `references/characters/<working-name>/base.png` and tell me when locked."* (This is the Mode 0 face-lock save step — the same convention applies to every downstream locked reference the user saves along the way.)
...
1. **Real subject with real photos available** → use the real photos as references. Study, mirror back the locked visual spec, confirm with user, then move straight to Mode 1 (outfit work, for characters) or Mode 3 (scene/environment work, for props, vehicles, and locations). No development phase needed, and no Mode 0 face-lock build required — the real photos ARE the development and already serve as the identity anchor.
2. **Existing AI-generated character/asset from prior work** → ask for the reference image(s). Study and lock from those. Same flow as the real-photos path — proceed to Mode 1 (outfit work) or whichever mode the user asked for.
3. **Fictional / stylized / original** (no real-world equivalent and no prior asset) → run the free-form development flow below.

**If real photos / existing references are provided:** study them visual-only. Lock — face, bone structure, skin tone, hair color and texture, identity markers, body proportions (for characters); silhouette, materials, signature details, mechanical/structural features (for props/vehicles); architecture, lighting register, atmospheric character (for environments). Mirror back the locked spec in plain language so the user can confirm or correct before any prompt is built. **Note especially any fine details visible in the real photos that should be preserved in generated downstream shots** — this prevents drift like an E39/E46 taillight slip, where a generation quietly swaps in the wrong model-year detail once the real reference stops being consulted closely. Wait for confirmation.

**If developing from scratch (no real-world equivalent and no prior reference):** for characters, development happens in two stages — first a text spec, then a face-lock build via Mode 0. Do NOT jump straight to outfit prompts. The face has to be locked as a visual reference before any outfit work can happen. For props, vehicles, and environments, development is free-form with no Mode 0 equivalent — lock the spec, then move straight to Mode 1 or Mode 3 as appropriate.

Stage 1 — text spec: let the user describe the subject in their own words — vibe, era, role, energy, look, references, whatever they want to share. Listen. Then mirror back a locked spec in plain language covering:
...
- Any key identity markers — piercings, scars, beauty marks, tattoos, signature jewelry (for characters)
...
Wait for confirmation or correction. Iterate on the text spec freely until the user says it's locked. Then move to Stage 2 (characters only).

Stage 2 (characters only) — Mode 0 face lock build (see Mode 0 section below). Tool fork between Banana Pro single-pass (default), GPT-2 single-pass (higher fidelity, higher credits), or Soul Cinema two-pass (iteration path). Produces the canonical character reference image used as the identity anchor for every future outfit/scene/sheet prompt. Always run this before any outfit work for a new character. No exceptions. Props, vehicles, and environments have no Mode 0 equivalent — once the text spec is locked, move straight to Mode 1 (if the subject is styled as an outfit/wardrobe element) or Mode 3 (if it's an environment or scene element).

**Multi-panel real-photo reference grids.** When working with a real subject that has multiple key angles or detail elements (e.g. a car cockpit with steering wheel + dashboard + shifter + center stack), collect a handful of real photos covering the different angles/details and treat them as **individual references**, not a tiled grid. Individual photos at full resolution let downstream video/image tools pick the relevant detail per shot. (Tiled grids are for *same-subject-multi-angle identity locking* like the 3-panel character sheet in Mode 2A; for *different-detail-of-the-same-subject* references, individual photos win.)

### Mode 0 — Face lock (new characters only)

See the Mode 0 section below. Tool fork: Banana Pro single-pass (default), GPT-2 single-pass (highest fidelity, higher credits, chest-up only), or Soul Cinema two-pass (cheap iteration — Soul Cinema face plate then Banana Pro 3:4 lock). All paths use mid-gray seamless (the locked default backdrop — white only on explicit request), flat shadowless lighting, and a locked baseline wardrobe (plain black camisole for women, plain black ribbed tank for men). Produces the canonical reference image. Run once per new character.
...
**Goal:** Produce the canonical face reference for the character. Identity only — no outfit considerations beyond a locked neutral baseline top, no environment, no posing direction. Just: a clean, locked face on an 18% gray seamless background with flat shadowless lighting that makes the skin read matte and cinema-placement-ready.
...
- **Backdrop:** mid-gray seamless studio (locked default)
- **Lighting:** flat and shadowless (locked default — matched equal fill from all sides, no key/shadow side)
...
**Canonical Step 0.B prompt structure:** Use the GPT-2 prompt structure documented in the GPT-2 section of this skill (Mode 4). Apply the same identity essentials, wardrobe lock, gray backdrop, and flat shadowless lighting as Step 0.A — just routed through the GPT-2 prompt grammar instead of the Banana Pro grammar.
...
Run a lean Soul Cinema generation to produce a clean face plate on mid-gray seamless with flat shadowless lighting. The plate is exploratory — identity essentials only, no makeup detail, no granular facial anatomy, no fine identity markers (those go into Step 0.2 where Banana Pro can actually hold them).
...
- **Backdrop:** mid-gray seamless studio (locked default)
- **Lighting:** flat and shadowless (locked default, matching Step 0.1)
- **Framing:** 3:4 headshot, forehead to upper chest, face filling most of the frame
...
Note: Lighting is intentionally flat and shadowless, matched equal fill from all sides — no full theatrical cinema stack at this stage, no dramatic three-point lighting, no key/shadow side. The outfit is the only subject. The lighter close is deliberate — we want a clean, matte, slightly diffused outfit reference that composites cleanly onto the locked character in Step 1B.2 without dragging cinema register baggage along. Run this in Soul Cinema. The user saves the result — that's the outfit reference for Step 1B.2.
...
- **Backdrop:** mid-gray seamless studio (locked default)
- **Lighting:** flat shadowless studio lighting (locked default — matched equal fill, no shadow side)
...
- Background: same studio backdrop across all six cells (default 18% gray seamless, matching the base reference) for consistency. Only swap to white-across-all-six-panels if the user explicitly asks for a white sheet (see the 18% GRAY SEAMLESS + FLAT GRADE section).
- Lighting: same flat shadowless grade across all six cells (per the 18% GRAY SEAMLESS + FLAT GRADE section) — identity stays locked when lighting is locked
...
3. **No `@image` tags or `<<<image_n>>>` placeholders.** Image attachment happens in the Higgsfield UI directly. The prompt is text-only. (Exception: Mode 5's locked outfit-replacement prompt uses `@image1`/`@image2` — that two-reference grammar is Banana Pro's own UI syntax and stands as written.)
...
8. **Locked flat grade baked in for Modes 0, 1, 2, 4, 5.** The locked flat grade closes every Mode 0, 1, 2, 4, and 5 character prompt (with Step 1B.1 outfit reference using the lighter close documented in that section, and Mode 5 using its own locked lean prompt). The cinema stack closes Mode 3 scene plates only — see rule 9.
...
- [ ] Mode selected (0 face lock / 1 single-image outfit / 2A three-panel sheet (default) / 2B six-panel (legacy) / 3A character scene / 3B environment plate / 4 GPT-2 / 5 outfit replacement) and rationale
