...
- **Yes / needs developing →** kick over to banana-pro-director's Step 0 character development flow. Return to Seedance once locked.
...
**Project palette hook (optional).** When `ai-film-director` (at Step 0.5) has locked a project-level hex palette with the producer, that palette does not get appended as a bare list anywhere — a bare palette line is exactly what DISTRIBUTED STYLE forbids. Instead the colors get distributed to their home blocks like every other style aspect: each swatch attaches to a specific fabric, surface, or light source in **World Plate**, and the grade description in **Camera Capture** names the same palette by role (shadow, midtone, highlight, accent) rather than by hex list. The point is one color signature holding across every shot in the project — so the same 4–6 swatches recur project-wide, distributed differently per scene depending on what's actually in frame, never repeated as a floating list.

Worked example, project palette `#1a2832` (deep teal shadow), `#4a5b6c` (cool steel midtone), `#d8c8a0` (warm dusty highlight), `#e89a5e` (sunset accent):

> World Plate: *"…the alley walls hold a deep teal shadow in the recessed brickwork, the wet pavement carrying a cool steel-blue midtone under the one working streetlamp, that lamp itself throwing a warm dusty-gold highlight across the puddle directly beneath it…"*
> Camera Capture: *"…color-negative grade built on the same signature — teal shadow, steel midtone, dusty-gold highlight, a sunset-amber accent held in the rim light — held consistent with the rest of the project…"*

If no palette was locked at Step 0.5, skip this entirely — the cinema mode's default grade carries the look.
...
## LIP-SYNC HOOK (music-video shots only)

One carve-out for music-video shots where a character is on-camera mouthing a lyric to a track. Three concrete mechanics, nothing more — this is not a general sync grammar.

**The lyric is character action, not audio.** It lands in **Movement**, in the character-motion layer, as mouth motion the subject performs — same layer as any other physical action, timestamped like the rest of the beat. The facial state that carries it (jaw, lips, eyes, the half-smile or intensity underneath) is pinned in that subject's **Subject Lock**, so the expression register is locked before the mouth motion plays out in Movement. **Sound Bed stays diegetic-only** — it never names the song, never describes music, never carries the lyric. The diegetic-audio rule is not suspended for music-video shots; the lyric is picture, not sound. Music is layered onto the render in post.

**Canonical pattern, verbatim:**

> *"lips visibly mouthing the words '[exact lyric]' with exaggerated clarity"*

Optional tail, context-specific to a stadium/performance register — use only when the scene calls for it: *"as if teaching the crowd the hook."*

**How to use it.** When the caller (typically `ai-film-director` at Step 5.0.5) flags a shot as a music-video sync shot and supplies the exact lyric, drop the canonical pattern into the character-motion layer of Movement at the beat where the mouth motion occurs. Example, mid-beat in a Movement paragraph:

> *"…@sol_ref holds the single microphone out steady at arm's length in her right hand, lips visibly mouthing the words 'we got, we got, we got control' with exaggerated clarity, a playful half-smile pulling at the corner of her mouth, the crowd's arms rising in the midground behind her…"*

**The audio-side mechanic is the orchestrator's job, not this skill's.** The producer attaches a short slice of the track containing this exact lyric to the host platform's audio reference slot (Higgsfield "elements list" / ArtCraft "Audio Ref"). This skill doesn't write that step — it's covered in `ai-film-director` Step 6.1 (Phase 6) and Step 5.3. This skill's only job is the in-prompt Movement pattern above.

**Don't invent additional sync mechanics.** The recipe is exactly three pieces: the slice-and-upload step (orchestrator's job), the audio-ref attachment (orchestrator's job), and this Movement pattern (this skill's job). No bar-alignment grammar, no tempo cues, no per-syllable timing. If a caller asks for sync mechanics beyond these three, refuse and explain the source doesn't cover it.
...
16. **Age-blind.** Never describe subjects by age. Avoid: *boy, girl, child, kid, young, teen, little, middle-aged, elderly, old.* Describe by role, hair, wardrobe, identity markers.
...
22. **No real brand names in prompt output.** Generic visual descriptors only — "black three-stripe athletic sneakers" not a brand name, "wide-angle action camera" not a product name. Internal chat with the user can reference brands by name; the prompt output stays brand-neutral to avoid moderation delays.
23. **English only inside the code block.**
...
- [ ] No character names, no platform names, no real brand names, no meta-commentary
